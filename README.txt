...
In order to search the walls for the indicated pattern, it is necessary to assign a set of these words to each wall, called wall labels. The wall labels are path dependent. Because of this, every allowable wall triple (last wall, current wall, next wall) is associated to a set of allowable wall labels using the information in (1)-(4) in the module walllabels.py. The wall labels are saved in a dictionary called wallinfo. wallinfo is keyed by the tuples (last wall, current wall) and contains a list of tuples [(next wall, [ wall labels ])...].

Now we can call matchPattern() in patternmatch.py. The heart of the algorithm is the function recursePattern() in the same module. The match is initialized with the tuples [(current wall, next wall)...] that have the first pattern element in the wall labels of wallinfo[(any last wall, current wall)] = [(next wall, [ wall labels ])...]. Then the algorithm  steps to the next matching wall (or allowable intermediate wall) from there. There is documentation and code commenting within the module. 
